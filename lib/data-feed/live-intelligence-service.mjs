...
export async function refreshLiveIntelligence() {
  ...
  try {
    ...
    // 5b. Nemotron Synthesis Wire: If NVIDIA NIM API key is configured, enrich top signals via live Nemotron model
    const nemotronStatus = checkNemotronStatus();
    if (nemotronStatus.configured) {
      console.log('  -> NVIDIA NIM detected! Calling Nemotron-70B API for live synthesis...');
      for (const sig of signals.slice(0, 3)) {
        try {
          const tickerDisclosures = congressData.disclosures.filter((d) => d.ticker === sig.ticker);
          const tickerNews = allArticles.filter((a) => a.relatedTickers?.includes(sig.ticker));
          const nemResult = await synthesizeWithNemotron({
            ticker: sig.ticker,
            companyName: sig.companyName,
            disclosures: tickerDisclosures,
            newsArticles: tickerNews,
          });
          if (nemResult?.success && nemResult.source === 'nvidia-nim' && nemResult.signal) {
            sig.aiModel = nemResult.signal.aiModel;
            sig.synthesisMode = 'NEMOTRON_NIM';
            sig.synthesisNote = nemResult.note;
            if (nemResult.signal.thesis) sig.thesis = `${nemResult.signal.thesis}\n\n[Synthesized via live NVIDIA NIM Llama-3.1-Nemotron-70B]`;
            if (nemResult.signal.headline) sig.headline = nemResult.signal.headline;
            if (nemResult.signal.keyRisks) sig.keyRisks = nemResult.signal.keyRisks;
          }
        } catch (nemErr) {
          console.warn(`  [Nemotron Live] Could not enrich ${sig.ticker}: ${nemErr.message}`);
        }
      }
    } else {
      console.log('  -> No NVIDIA_API_KEY detected. Signals labeled transparently under Heuristic Fallback mode.');
    }
...
export { checkNemotronStatus as getNemotronStatus } from '../../src/services/nemotronService.mjs';
