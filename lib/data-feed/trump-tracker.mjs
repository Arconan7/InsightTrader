...
export const getCachedTrumpPosts = loadTrumpPosts;
...
export function calculateTrumpContextScore(ticker, posts = null) {
  ...
  const postDate = topPost?.publishedAt ? new Date(topPost.publishedAt).toLocaleDateString() : 'Recent';
  const postSnippet = (topPost?.content || topPost?.headline || '').slice(0, 100);
  const rationale = `Recent public statement (${postDate}): "${postSnippet}..." (${sentiment} context; 5% verdict weight).`;
